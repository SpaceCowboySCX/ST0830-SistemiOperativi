...
int hiding(char* file_img_dot, char* file_txt_dot, char* file_img_output_dot){
    ...
    for (int index = 0; index < offset; index++) {
        ...
        fwrite(&byte_img, sizeof (char), 1, file_img_output);
    ...
    while (!feof(file_img)) {
        ...
        if (!feof(file_txt)) {
            ...
            fread(&byte_text, sizeof (char), 1, file_txt);
            ...
            for (int index = 0; index < 8; index++) {
                ...
                fwrite(&to_write, sizeof (char), 1, file_img);
            ...
            fread(&copy_of_byte_img, sizeof (char), 1, file_img);
            copy_of_byte_img &= 0xFE;
            //fprintf(file_img_output, "%c", copy_of_byte_img);
            fwrite(&copy_of_byte_img, sizeof (char), 1, file_img_output);
        }
    }
    fclose(file_img);
...
void toSkip(FILE* file_img, FILE* file_img_output){
    ...
    fread(&to_skip, sizeof (char), 1, file_img);
    //fprintf(file_img_output, "%c",to_skip);
    fwrite(&to_skip, sizeof (char), 1, file_img_output);
}

int unveiling(char* file_img_dot, char* file_txt_output_dot) {
    FILE* file_img = fopen(file_img_dot, "rb");
    FILE* file_txt_output = fopen(file_txt_output_dot, "ab");
    byte byte_img = 0x00;
    byte byte_text;
    ...
    while (!feof(file_img)) {
        ...
        for (int index = 0; index < 8; index++) {
            ...
            if ((index + 1) % 3 == 0)
                fseek(file_img, sizeof (char), SEEK_CUR);
        }

        fseek(file_img, sizeof (char), SEEK_CUR);
        //fprintf(file_txt_output, "%c", byte_text);
        fwrite(&byte_text, sizeof (char), 1, file_txt_output);
